...
const Generator: React.FC = () => {
  ...
  const { sources, loading: reviewsLoading, fetchReviewSources } = useReviews();
  const { currentBrandId, config: brandConfigData, fetchBrand } = useBrand();
  const {
    isGenerating, currentQuote, selection, generatedCreatives,
    generateCreative, setFormat, setSocialPlatform, setCta, setCurrentQuote
  } = useCreatives();
  const { format, socialPlatform, cta } = selection;
  const brandColors = brandConfigData?.colors || { primary: '#6366f1', secondary: '#8b5cf6', text: '#1f2937', background: '#ffffff', accent: '#404040' };
  const brandLogos = brandConfigData?.logos || {};
  const brandTypography = brandConfigData?.typography || { fontFamily: 'Inter' };

  // Safe brands placeholder with correct flat structure for UI
  const brands: any[] = currentBrandId && brandConfigData ? [{
    id: currentBrandId,
    name: brandConfigData.name,
    primary_color: brandConfigData.colors.primary,
    secondary_color: brandConfigData.colors.secondary,
    accent_color: brandConfigData.colors.accent,
    primary_logo: brandConfigData.logos.primary
  }] : [];

  const currentBrand = brands.length > 0 ? brands[0] : null;
  ...
  }, [fetchTestimonials, fetchReviewSources, fetchBrand]);



  // Combine testimonials and reviews (same logic as Library.tsx)
  // Combine testimonials and reviews (same logic as Library.tsx)
  const reviewsByBrand: Record<string, { brand: any; reviews: any[]; sources: typeof sources }> = {};

  // Single brand logic: just put everything under current brand if it exists
  if (currentBrand) {
    const brandSources = sources.filter(s => s.brandId === currentBrand.id);
    ...
    if (brandReviews.length > 0) {
      reviewsByBrand[currentBrand.id] = {
        brand: currentBrand,
  ...
  const allTestimonials: Array<Testimonial & { sourceType?: string; sourceId?: string; brandId?: string; brandName?: string }> = [
    ...
    ...(Object.values(reviewsByBrand) as any[]).flatMap(({ reviews }) =>
      reviews.map((r: any) => ({
        id: r.id,
  ...
  // Under single brand logic, we always show everything available (which should be current brand's items + unassigned)
  ...
  useEffect(() => {
    // Note: location.state is not supported in Next.js App Router.
    // Logic removed or needs query param implementation.
  ...
  useEffect(() => {
    if (selectedTestimonial) {
      setCurrentQuote(selectedTestimonial.content);
    }
  }, [selectedTestimonialId, setCurrentQuote, selectedTestimonial]);
  ...
  const handleGenerate = (additionalPrompt?: string) => {
    ...
    const runGenerationFlow = async () => {
      try {
        ...
        generateCreative(generatePayload)
          .then((result: any) => {
            clearInterval(progressInterval);
    ...
    return (
      <div className="flex flex-col lg:flex-row h-full gap-8">
        <div className="w-full lg:w-[420px] bg-white border border-gray-200 rounded-xl p-6 flex flex-col gap-8 shadow-sm h-fit">
          ...
          {/* Brand Selector Removed for Single Brand Mode */}
        </div>
        <div className="flex-1 bg-background-light border border-gray-200 rounded-2xl flex items-center justify-center min-h-[500px]">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {['Instagram', 'LinkedIn', 'Twitter', 'Facebook'].map((p) => (
              <button
                key={p}
                onClick={() => setSocialPlatform(p as SocialPlatform)}

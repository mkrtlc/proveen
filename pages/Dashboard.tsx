...
import { setPlatform, setCta, generateCreative } from '../store/slices/creativeSlice';
import { useNavigate } from 'react-router-dom';
import { supabase } from '../lib/supabase';
import { getTimeBasedGreeting } from '../lib/utils';
import NameModal from '../components/NameModal';
import { StatCardSkeleton, CardGridSkeleton, ActivityItemSkeleton } from '../components/Skeleton';
...
const Dashboard: React.FC = () => {
  const { generatedCreatives, loading: creativesLoading } = useAppSelector((state) => state.creative);
  const { items: testimonials, loading: testimonialsLoading } = useAppSelector((state) => state.testimonials);
  const { sources: reviewSources } = useAppSelector((state) => state.reviews);
  const { config: brandConfig } = useAppSelector((state) => state.brand);
  const navigate = useNavigate();
  const [userName, setUserName] = useState<string>('');
  const [greeting, setGreeting] = useState<string>('Good morning');
  const [showNameModal, setShowNameModal] = useState(false);

  const isLoading = creativesLoading || testimonialsLoading;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button className="text-sm font-medium text-primary hover:text-primary-light flex items-center gap-1">
                ...
                <div key={creative.id} className="group relative bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all cursor-pointer">
                  <div className="aspect-[4/5] w-full bg-gray-100 relative">
                    ...
                    <div className="absolute top-3 right-3 bg-black/50 backdrop-blur-sm rounded-full p-1.5">
                      <Icon name={creative.platform === 'Post' ? 'photo_camera' : 'public'} size={16} className="text-white" />
                    </div>
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-xs text-gray-500 mt-1">Generated {creative.timestamp} • {creative.platform}</p>
                  </div>
                </div>
              ))}
            </div>

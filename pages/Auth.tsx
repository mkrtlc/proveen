    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isSignUp) {
                ...
                // but user.email_confirmed_at will be null until confirmed
                if (data.user && !data.session) {
                    // Email confirmation required
                    setError(null);
                    alert('Confirmation email sent! Please check your inbox and click the confirmation link.');
                } else if (data.session) {
                    // Auto-login (email confirmation disabled in Supabase settings)
                ...
                if (error) throw error;
                // Check if user has a name (for existing users who might not have set it)
                const userName = data.user?.user_metadata?.name;
                if (!userName) {
                    setShowNameModal(true);
                } else {
